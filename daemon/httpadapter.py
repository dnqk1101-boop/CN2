...
class HttpAdapter:
    ...
    async def handle_client_coroutine(self, reader, writer):
        ...
        if req.method == "OPTIONS":
            ...
            response = resp.build_response(req, envelop_content=b"")
        elif req.hook:
            import inspect
            if inspect.iscoroutinefunction(req.hook):
                hook_resp = await req.hook(req, resp)
            else:
                hook_resp = req.hook(req, resp)
            # For now we use build_response
            response = resp.build_response(req, envelop_content=hook_resp)
        else:
            # Build response
