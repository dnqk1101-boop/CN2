...
class Request():
    ...
    def prepare(self, request, routes=None):
        ...
        self._raw_headers, self.body = self.fetch_headers_body(request)
        ...
        if not routes == {} and self.method and self.path:
            self.routes = routes
            path_no_query = self.path.split('?')[0]
            print("[Request] Routing METHOD {} path {}".format(self.method, self.path))
            self.hook = routes.get((self.method, path_no_query))
            if self.hook is None and path_no_query.endswith('/'):
                self.hook = routes.get((self.method, path_no_query[:-1]))
        ...
        self._raw_heaers = self._raw_headers
