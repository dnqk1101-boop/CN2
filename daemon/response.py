...
class Response():   
    ...
    def build_content(self, path, base_dir):
        ...
        filepath = os.path.join(base_dir, path_no_query.lstrip('/'))
    ...
    def build_response_header(self, request):
        ...
        status = "200 OK"
        if getattr(self, "status_code", None) == 401:
            status = "401 Unauthorized"
        elif getattr(self, "status_code", None) == 400:
            status = "400 Bad Request"
            
        fmt_header = f"HTTP/1.1 {status}\r\n"
        for k, v in headers.items():
            fmt_header += f"{k}: {v}\r\n"
        
        for k, v in self.cookies.items():
            fmt_header += f"Set-Cookie: {k}={v}\r\n"
    ...
    def build_response(self, request, envelop_content=None):
        ...
        if getattr(request, 'method', '').strip().upper() == "OPTIONS":
